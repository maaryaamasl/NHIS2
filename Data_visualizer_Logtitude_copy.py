if False:
    ...
    top_n = 20
    take_abs = True
    outcome = "pain_trajectory"

    # -------------------------------
    # Load feature names and class names
    feature_names = pd.read_csv(os.path.join(shap_dir, 'columns.csv'))['Column Names'].values
    class_names = pd.read_csv(os.path.join(shap_dir, 'class_names.csv'))['Class Names'].values
    num_samples = int(np.loadtxt(os.path.join(shap_dir, 'shape.csv')))
    ...
    # -------------------------------
    # Load SHAP values [samples, features, classes]
    shap_values_array = np.zeros((num_samples, num_features, num_classes))
    for i in range(num_samples):
        shap_values_array[i] = np.loadtxt(os.path.join(shap_dir, f'shap_{i}.csv'))
    ...
    print("Mean SHAP shape:", mean_shap_values.shape)

    # -------------------------------
    # Load variable descriptions and categories
    ...
    def create_label(inx):
        parts = inx.split('__')
        base_var = parts[0]
        if base_var in column_desc:
            desc = column_desc[base_var]
        else:
            desc = f"[Unknown Variable: {base_var}]"

        if len(parts) == 2:
            return f"{desc} [{base_var}] ({parts[1]})"
        else:
            return f"{desc} [{base_var}]"


    # -------------------------------
    # Plotting SHAP values per class
    for class_idx, class_name in enumerate(class_names):
        print(f"\nPlotting for class: {class_name}")

        df = pd.DataFrame({
            'Feature': feature_names,
            'Mean_SHAP': mean_shap_values[:, class_idx]
        }).sort_values(by='Mean_SHAP', ascending=False).head(top_n)

        df['In_Description'] = df['Feature'].apply(lambda x: x.split('__')[0] in column_desc)
        print(df[['Feature', 'Mean_SHAP', 'In_Description']])

        if take_abs:
            df = df.sort_values(by='Mean_SHAP', ascending=False).head(top_n)
        else:
            df = df.reindex(df['Mean_SHAP'].abs().sort_values(ascending=False).head(top_n).index)

        df['Label'] = df['Feature'].apply(create_label)
        df['Category'] = df['Feature'].str.split('__', expand=True)[0].map(column_cat)

        plt.figure(figsize=(10, 6))
        sns.barplot(data=df, x='Mean_SHAP', y='Label', hue='Category', dodge=False)
        plt.title(f"Top {top_n} SHAP Features for Class: {class_name}")
        plt.xlabel('Mean |SHAP| (impact on output)')
        plt.ylabel('Features')
        plt.legend(title='Category', bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()

        save_path = os.path.join(shap_dir, f"SHAP_Barplot_{class_name}_Top{top_n}.png")
