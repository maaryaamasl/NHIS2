...
def print_hi(name):
    ...
    ################################################################################
    for file in ['./adult19.csv','./adult20.csv']:
        print("##################################\n",file,"\n##################################")
        
        df = pd.read_csv(file,dtype={'HHX': str})
        ...
        # 2020
        # Mssing Columns: PHQCAT_A ,GADCAT_A ,MEDDNG12M_A <<<<<<<<<<<<<<<<<<<<<
        
        missing_column = ['MEDDNG12M_A'] # , 'FSNAP12M_Z
        if file == './adult20.csv':
            missing_column.extend(['PHQCAT_A' ,'GADCAT_A'])
        selected_columns = [x for x in selected_columns if x not in missing_column]
        if "HHX" in selected_columns:
            print('HHX found in selected_columns')
        selected_data = df[selected_columns].copy()
        selected_data['HHX'] = selected_data['HHX'].astype(str)
        ...
        print("missing30")
        # 2019 
        #  ANXLEVEL_A  DEPLEVEL_A  PAYNOBLLNW_A  EMPWKHRS2_A  EMPWRKFT_A  FSNAP30D_A  HOUGVASST_A
        #   9889       17232         27367        12525       12528       28096        21360
        # 2020
        # DEPLEVEL_A  PAYNOBLLNW_A  EMPWKHRS2_A  EMPWRKFT_A  FSNAP30D_A  HOUGVASST_A
        # 16822         27804        13749       13731       28154        22362
        ...
        ############################# ageEdu ####################################
        bins = [0, 24, 34, 44, 54, 64, 74, float('inf')]
        labels = ['Age less than 25', 'Age 25-34', 'Age 35-44', 'Age 45-54', 'Age 55-64', 'Age 65-74', 'Age 75+']
        selected_data['AGEP_A'] = pd.cut(selected_data['AGEP_A'], bins=bins, labels=labels, right=True)
        education_mapping = {
        ...
        selected_data['EDUC_A'] = selected_data['EDUC_A'].map(education_mapping)
        selected_data['MAXEDUC_A'] = selected_data['MAXEDUC_A'].map(education_mapping)

        # Make categorical
        if make_categorical:
            for column in ['REGION', 'ORIENT_A', 'MARITAL_A', 'RACEALLP_A','AGEP_A','EDUC_A','MAXEDUC_A']: # modified new: removed 'HISPALLP_A'
        ...
        selected_data.to_csv('Cleaned_data_20'+year+'_ageEdu.csv', index=False, header=True)
